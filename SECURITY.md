...
Those requests send a query string, IP address, and the user agent `Rebost/0.9.6 (local-first open-source desktop AI; https://github.com/Frontierz-AI/Rebost)`. They do not include Shelf documents.

With Online on in Settings, Chat can also look things up on the public web. Each query or address is shown for approval before it is sent. Approved lookups leave the machine directly and do not go through Rebost, and they carry a user agent naming the project and a contact address. Chat is asked not to put private details in them.
