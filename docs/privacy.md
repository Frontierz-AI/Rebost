...
| Chat text is not uploaded | **True while Online is off.** With Online on, the search query or page URL Chat writes leaves the machine only after that exact text is approved. Those lookups do not go through Rebost. Prompts tell Chat not to put Shelf text or personal details in them. That is an instruction, not a filter. |
...
| `huggingface.co` | Explore / install | Search query, IP, and the user agent `Rebost/0.9.2 (local-first open-source desktop AI; https://github.com/Frontierz-AI/Rebost)` |
...
| `en.wikipedia.org`, `api.duckduckgo.com`, `api.you.com` | Chat, only if Settings → Online is on and the exact query is approved | The search query Chat writes; IP; and the user agent `Rebost/0.9.2 (https://github.com/Frontierz-AI/Rebost; info-rebost-app@frontierz.com)`, which names the project and a contact address. Prompts ask Chat not to put Shelf text or personal details in the query. |
