...
| `huggingface.co` | Explore / install | Search query, IP, and the user agent `Rebost/0.8.13 (local-first open-source desktop AI; https://github.com/Frontierz-AI/Rebost)` |
...
| `en.wikipedia.org`, `api.duckduckgo.com`, `api.you.com` | Chat, only if Settings → Online is on | The search query Chat writes; IP; and the user agent `Rebost/0.8.13 (https://github.com/Frontierz-AI/Rebost; info-rebost-app@frontierz.com)`, which names the project and a contact address. Prompts ask Chat not to put Shelf text or personal details in the query. |
