...
| `huggingface.co` | Explore / install | Search query, IP, `Rebost/0.6.0` user agent |
...
| `en.wikipedia.org`, `api.duckduckgo.com`, `api.you.com` | Chat, only if Settings → Online is on | The search query Chat writes; IP; `Rebost/0.6.0` user agent. Prompts ask Chat not to put Shelf text or personal details in the query. |
