...
const IDENTIFIER: &str = "io.rebost.app";
...
async fn main() -> anyhow::Result<()> {
    ...
    if fresh && data_dir.exists() {
        ...
        println!("wiped {}", data_dir.display());
    }
    ...
    // Demo shelves live under app data so they stay out of ~/Documents/Rebost.
    let shelf_root = data_dir.join("DemoShelves");
    {
        let mut settings = ctx.settings.write().unwrap();
        settings.shelf_root = Some(shelf_root.clone());
...
"#,
    ...
    for (shelf_name, files) in [
        ...
        let (_, copied) = rebost::shelf::import_into_shelf(&shelf, &files)?;
        for file in copied {
            let rel = file
