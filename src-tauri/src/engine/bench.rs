...
/// Wait until Chat is idle, then measure once per installed model.
pub fn schedule_after_chat(engine: &Arc<Engine>, base_url: String) {
    if !needs_benchmark(engine) {
        return;
    }
    let engine = engine.clone();
    tokio::spawn(async move {
        tokio::time::sleep(Duration::from_secs(3)).await;
        run_if_needed(&engine, &base_url).await;
    });
}

fn needs_benchmark(engine: &Engine) -> bool {
    let settings = engine.ctx_settings();
    let Some(model) = &settings.active_model else {
        return false;
    };
    settings
        .benchmark
        .as_ref()
        .map(|b| b.model_file != model.file)
        .unwrap_or(true)
}
...
pub async fn run_if_needed(engine: &Arc<Engine>, base_url: &str) {
    if engine.generation_in_flight.load(Ordering::Relaxed) != 0 {
        return;
    }
