...
async fn full_loop_answers_with_citations() {
    ...
    // ── Model install (local file — the download path is exercised in the
    //    app; here we verify the activation contract) ─────────────────────
    ...
    // ── Engine up (installs the pinned build from the local archive) ────
    ...
    // ── Conversation memory: the exchange is indexed and searchable ─────
    let memories = app
        .ctx
        .search
    ...
    // ── General question with No Shelf still answers ─────────────────────
