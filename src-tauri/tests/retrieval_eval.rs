...
fn gated(corpus: &Corpus, query: &str) -> Vec<rebost::types::SourcePassage> {
    ...
    let hits = corpus
        ...
        .ctx
        .search
        .search_passages(query, &corpus.shelf_id, 24)
        .expect("search");
    gate::gate_passages(hits, &tokens)
}

#[tokio::test(flavor = "multi_thread")]
async fn termination_question_finds_the_termination_section() {
    let corpus = corpus().await;
    let results = gated(&corpus, "When can Santander terminate our agreement?");
...
async fn typos_still_match_via_fuzzy() {
    ...
    assert!(
        results.iter().any(|r| r.title.contains("Santander")),
        "expected the Santander agreement among results"
    );
}
...
async fn unrelated_queries_are_gated_out() {
    ...
    let results = gated(
        &corpus,
        "quantum entanglement in photosynthesis experiments",
    );
    assert!(
        results.is_empty(),
...
async fn shelf_scoping_is_strict() {
    ...
    let hits = app
        .ctx
        .search
        .search_passages("termination notice Santander", &finance, 24)
        .expect("search");
...
async fn budget_fitting_respects_the_measured_budget() {
    ...
    let tokens = corpus
        ...
        .ctx
        .search
        .query_tokens("Santander agreement payment termination");
    let hits = corpus
        .app
        .ctx
        .search
        .search_passages(
            "Santander agreement payment termination",
            &corpus.shelf_id,
            24,
        )
        .unwrap();
